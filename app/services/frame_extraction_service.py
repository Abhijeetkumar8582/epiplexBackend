...
class FrameExtractionService:
    ...
    def extract_frames_opencv(
        ...
        try:
            ...
            if not cap.isOpened():
                raise Exception(f"Could not open video file: {video_path}. File may be corrupted or format not supported.")
            
            fps = cap.get(cv2.CAP_PROP_FPS)
            if fps == 0 or fps is None:
                raise Exception(f"Invalid video FPS: {fps}. Video file may be corrupted.")
            
            # Check if video has any frames
            total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            if total_frames == 0:
                raise Exception(f"Video file has no frames: {video_path}. File may be corrupted or empty.")
            ...
            logger.info("Starting frame extraction", video_path=video_path, fps=fps, interval=frame_interval, total_frames=total_frames)
            ...
            # Always release video capture, even on error
            if cap is not None:
                cap.release()
